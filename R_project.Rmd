...
knitr::opts_chunk$set(tidy=FALSE, echo=TRUE, messages=FALSE, warnings=FALSE, cache=FALSE)
knitr::opts_knit$set(root.dir="~/Dropbox/Work/DataFest/2021")
...
packages <- c("viridis", "glmmTMB",  "effects", "dataverse", "sf", 
...
As an example of how to use the Dataverse API, we're going to extract daily data on COVID-19 cases from the U.S. at the state-level (from dataset #1 above). These data span the period from January 21st 2020 until November 29th 2020 for each U.S. state (and the District of Columbia). 
...
We can now examine the structure of the data if we uncomment the line with `glimpse(US_cases)` (warning --- the print out is long):
```{r}
# inspect the data (warning --- the print out is long)
# glimpse(US_cases) # 50 states plus DC by 314 days
```
...
This section is optional. It provides an example of how to acquire potentially interesting predictors of COVID-19 cases.
...
# update the death rate variables to use population estimates from 2019
...
US_cases_long_week_spatial %>% 
  ...
  labs(title = "COVID-19 cases for last week",
       caption = "Data Source: U.S. Census Bureau; 2019")
```
...
USmap <- US_cases_long_week_spatial %>% 
  ...
  filter(week_of_year == max(week_of_year, na.rm = TRUE)) %>%
  # map starts here
  mapview(zcol = "cases_rate_100K", layer.name = "Cases (rate per 100K)")
...
```{r}
load("US_cases_long_demogr_week.Rdata")
...
So far, however, we've only been modeling a linear trend for time. From our visualizations we know this is unrealistic. How could we incorporate non-linear time elements in the model?
